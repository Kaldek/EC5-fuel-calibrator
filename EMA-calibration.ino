...
void loop() {
  ...
  if ((fuelLevel != lastFuelLevel) && (currentTime - lastUpdateTime >= interval)) {
    if (fuelLevel <= 6) {
      lightFlag = 3; // Fast flash
    } else if (fuelLevel <= 8) {
      lightFlag = 2; // Slow flash
  ...
  switch (lightFlag) {
    ...
    case 2:
      digitalWrite(Light, HIGH); delay(600);
      digitalWrite(Light, LOW); delay(600); break;
    case 3:
      digitalWrite(Light, HIGH); delay(200);
      digitalWrite(Light, LOW); delay(200); break;
    default:
